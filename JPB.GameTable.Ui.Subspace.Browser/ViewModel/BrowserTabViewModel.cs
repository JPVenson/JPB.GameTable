...
namespace JPB.GameTable.Ui.Subspace.Browser.ViewModel
{
	...
	{
		private void Set<T>(ref T field, T ne, [CallerMemberName]string caller = null)
		{
			SetProperty(ref field, ne, caller);
		}
		...
		public BrowserTabViewModel(string address)
		{
			...
			HomeCommand = new DelegateCommand(() => AddressEditable = Address = "https://www.google.de");
			...
			ShowDevToolsCommand = new DelegateCommand(() => webBrowser.ShowDevTools());
			CloseDevToolsCommand = new DelegateCommand(() => webBrowser.CloseDevTools());
